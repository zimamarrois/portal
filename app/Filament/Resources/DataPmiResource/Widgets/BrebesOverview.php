...
class BrebesOverview extends BaseWidget
{
    protected function getCards(): array
    {
        $DATAPMI = DataPmi::where('kantor_id', '3')->count();
        $MEDICAL = DataPmi::where('kantor_id', '3')->where('medical_check','1')->count();
        
        $NONJOB = DataPmi::where('kantor_id', '3')->where('job','0')->count();
        $JOB = DataPmi::where('kantor_id', '3')->where('job','1')->count();
        
        $PMIFINISH = DataPmi::where('kantor_id', '3')->where('status_update_id','18')->count();
        $PMIMD = DataPmi::where('kantor_id', '3')->where('status_update_id','19')->count();
        $PMIPENDING = DataPmi::where('kantor_id', '3')->where('status_update_id','21')->count();
        $FIT = DataPmi::where('kantor_id', '3')->where('fit','1')->count();
        $UNFIT = DataPmi::where('kantor_id', '3')->where('fit','0')->count();
        $TERBANG = DataPmi::where('kantor_id', '3')->where('status_update_id','15')->count();
        
        $TERDAFTARSIAPKERJA = DataPmi::where('kantor_id', '3')->where('siapkerja','1')->count();
        $VERDATA = DataPmi::where('kantor_id', '3')->where('verdata','1')->count();
        $VERPP = DataPmi::where('kantor_id', '3')->where('verpp','1')->count();
